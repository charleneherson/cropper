...
export const _isInApp = (function () {
  return (browser.android || browser.iPhone || browser.iPad) && (userAgent.indexOf('_vc')>0);
})()
...
export function canUseCommon(API: any ): boolean{
  ...
  const isNewJsBridge = +switchConfig.jsBridge_isNewJsBridge;
  const vcnum = switchConfig.jsBridge_vcnum;
  const hasZYBJSBridge = win.webkit && win.webkit.messageHandlers && win.webkit.messageHandlers.ZYBJSBridge;
  let canUseCommon: boolean = isNewJsBridge === 1 && vcnum >= 210 && hasZYBJSBridge;

  if(checkSystem.isAndroid()){
