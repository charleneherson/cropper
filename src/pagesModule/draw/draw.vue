<template>
  ...
  >
    ...
    <div class="fixed-bottom bg-primary box-shadow-top">
      ...
      <image style="width: 160rpx; height: 160rpx; " src="https://zyb-yayayuwen-1253445850.cos.ap-beijing.myqcloud.com/yaya-fe/steps/bg_submerge_1.png" alt="img" />
      <image style="width: 160rpx; height: 160rpx; " src="https://csdnimg.cn/cdn/content-toolbar/csdnlogo.png" alt="img" />
      ...
      <a class="position-absolute reset bg-primary border-light d-flex justify-content-center align-items-center shadow" @click="reset">
        <image src="https://zyb-yayayuwen-1253445850.cos.ap-beijing.myqcloud.com/yaya-fe/OWebsite/icon_triangle_white_left.png" style="width: 80rpx; height: 80rpx;" alt="reset" />        
      </a>
      <a class="position-absolute done bg-primary border-light d-flex justify-content-center align-items-center shadow" @click="next">
        <image src="https://zyb-yayayuwen-1253445850.cos.ap-beijing.myqcloud.com/yaya-fe/OWebsite/icon_triangle_white_left.png" style="width: 80rpx; height: 80rpx;" alt="next"/>
      </a>-->
      ...
      <!-- <scroll-view scroll-x class="py-2" style="width: 100vw; height: 224rpx;">
        <div class="d-flex px-2 justify-content-between flex-nowrap px-2">
          <view
            ...
            :item="item"
            :key="item+index"
            class="m-2 p-2 bg-light rounded shadow item"
          >
            {{ item.path }}
            
            <button v-if="item.isAuth" open-type="getUserInfo" @getuserinfo="getuserinfo" style="background-color: transparent;">
              <image class="shadow-sm" style="width: 160rpx; height: 160rpx; overflow: hidden;" src="https://zyb-yayayuwen-1253445850.cos.ap-beijing.myqcloud.com/yaya-fe/steps/bg_submerge_1.png" alt="img" />
            </button>
            <view v-else @click="select2(index)">
              <image style="width: 160rpx; height: 160rpx; overflow: hidden;" src="https://zyb-yayayuwen-1253445850.cos.ap-beijing.myqcloud.com/yaya-fe/steps/bg_submerge_1.png" alt="img" />
            </view>
          </view>
        </div>
      </scroll-view> -->
    ...
    touchstart(e) {
      ...
      for (
        ...
        del.x =
          center.x +
          r *
            Math.sin((Math.PI / 180) * (-45 - item.rotate));
        del.y =
          center.y -
          r *
            Math.cos((Math.PI / 180) * (-45 - item.rotate));

        move.y =
          center.y +
          r *
            Math.sin((Math.PI / 180) * (135 - item.rotate));
        move.x =
          center.x -
          r *
            Math.cos((Math.PI / 180) * (135 - item.rotate));
        if (item && item.active) {
          // 判断删除
          if (
      ...
      if (this.touch.method === "drag") {
        if (
          ...
          this.elements[this.touch.element.index].x =
            this.touch.element.x +
            this.touch.move.x -
            this.touch.start.x;
          this.elements[this.touch.element.index].y =
            this.touch.element.y +
            this.touch.move.y -
            this.touch.start.y;
        }
        ...
        let centerX =
          this.touch.element.x +
          this.touch.element.width / 2;
        let centerY =
          this.touch.element.y +
          this.touch.element.height / 2;
        ...
        let angleBefore =
          (Math.atan2(diffYBefore, diffXBefore) / Math.PI) *
          180;
        let angleAfter =
          (Math.atan2(diffYAfter, diffXAfter) / Math.PI) *
          180;
